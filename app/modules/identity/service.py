...
class AuthService:
    ...
    async def register(self, data: RegisterRequest) -> TokenPair:
        if await user_repo.get_by_email(self._db, data.email):
            raise ConflictError("Пользователь с таким email уже существует")
        user = await user_repo.create(self._db, UserCreate(**data.model_dump()))
        return await self._issue_tokens(user)

    async def login(self, data: LoginRequest) -> TokenPair:
        user = await user_repo.get_by_email(self._db, data.email)
        if not user or not user_repo.authenticate(user, data.password):
    ...
    async def refresh(self, data: RefreshRequest) -> TokenPair:
        ...
        user = await user_repo.get_by_id(self._db, user_id)
        if not user:
