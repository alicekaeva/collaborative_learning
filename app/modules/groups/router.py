...
from app.modules.groups.schemas.group import (
...
async def list_groups(
    ...
    if tag_id:
        return await group_repo.get_by_tag(db, tag_id, skip=skip, limit=limit)
    if category_id:
        return await group_repo.get_by_category(db, category_id, skip=skip, limit=limit)
    return await group_repo.get_all(db, skip=skip, limit=limit)
...
async def get_group(group_id: int, db: DBDep):
    group = await group_repo.get_by_id_with_details(db, group_id)
    if not group:
