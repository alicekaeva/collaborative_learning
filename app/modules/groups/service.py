...
from app.modules.groups.schemas.group import GroupCreate, GroupUpdate, GroupRead
...
class GroupService:
    ...
    async def create_group(self, data: GroupCreate, current_user: User) -> Group:
        ...
        return await group_repo.create(self._db, data, current_user.admin_profile)

    async def update_group(self, group_id: int, data: GroupUpdate, current_user: User) -> Group:
        group = await self._get_or_404(group_id)
        role = await group_repo.get_user_role_in_group(self._db, group, current_user)
        ...
        return await group_repo.update_group(self._db, group, data)
    ...
    async def delete_group(self, group_id: int, current_user: User) -> None:
        ...
        await group_repo.delete(self._db, group)
    ...
    async def add_member(self, group_id: int, user_id: int, role: str) -> Group:
        ...
        if role == "teacher":
            ...
            await group_repo.add_teacher(self._db, group, target_user.teacher_profile)
            ...
            await group_repo.add_student(self._db, group, target_user.student_profile)

        else:
            raise BadRequestError("Роль должна быть 'teacher' или 'student'")

        await self._db.commit()
        return await group_repo.get_by_id_with_details(self._db, group_id)
    ...
    async def remove_member(self, group_id: int, user_id: int, role: str) -> None:
        ...
        if role == "teacher" and target_user.teacher_profile:
            await group_repo.remove_teacher(self._db, group, target_user.teacher_profile)
            ...
            if not remaining.scalars().first():
                await user_repo.remove_role(self._db, target_user, "ROLE_TEACHER")
        elif role == "student" and target_user.student_profile:
            await group_repo.remove_student(self._db, group, target_user.student_profile)
            remaining = await self._db.execute(
                select(Group).where(
            ...
            if not remaining.scalars().first():
                await user_repo.remove_role(self._db, target_user, "ROLE_STUDENT")
        else:
    ...
    async def enroll_request(
        ...
        await msg_repo.send_direct(self._db, content, requester.id, admin_user_id)
    ...
    async def get_recommended(self, current_user: User) -> List[GroupRead]:
        ...
        groups = await group_repo.get_recommended(self._db, tag_ids)
    ...
    async def get_user_role(self, group_id: int, current_user: User) -> Optional[str]:
        group = await self._get_or_404(group_id)
        return await group_repo.get_user_role_in_group(self._db, group, current_user)

    # ------------------------------------------------------------------ helpers

    async def _get_or_404(self, group_id: int) -> Group:
        group = await group_repo.get_by_id(self._db, group_id)
        if not group:
